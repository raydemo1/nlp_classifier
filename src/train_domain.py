...
def setup_logging(log_dir="outputs/logs"):
    ...
    log_file = Path(log_dir) / f"train_domain_{int(time.time())}.log"
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for line in tqdm(lines, desc="加载数据集", unit="条"):
            ...
            if x.get("domain"):
                items.append(x)
    if logger:
        logger.info(f"加载了 {len(items)} 条有效的domain数据")
    return items
...
def build_label_map(items, logger=None):
    ...
    if logger:
        logger.info(f"构建了 {len(label_map)} 个domain标签: {list(labels)}")
    return label_map
...
def train(input_path="outputs/dataset.jsonl", outdir="outputs/models/domain", batch_size=32, epochs=6, lr=2e-5, device=None):
    ...
    print(f"✅ Domain模型训练完成，保存到: {outdir}")
    return outdir
